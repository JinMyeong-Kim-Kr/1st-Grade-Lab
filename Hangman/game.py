...
class HangmanGame(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.currentWord.setFixedWidth(280) #fix bug 폭을 늘려줌
    ...
    def startGame(self):
        ...
        self.guess = Guess(self.word.randFromDB(3)) #3 이상 단어만
        self.gameOver = False
    ...
    def guessClicked(self):
        ...
        if len(guessedChar) != 1:
            self.message.setText("Enter length 1")
            return
        ...
        if self.guess.finished():
            self.guessedChars.setText("Success!")
            self.gameOver = True
            # 메시지 ("Success!") 출력하고, self.gameOver 는 True 로


        elif self.hangman.getRemainingLives() == 0:
            self.guessedChars.setText("Fail! ," + "Secret Word = " + self.guess.secretWord)
            self.gameOver = True
        ...
        if self.gameOver == True:
            ...
            self.message.setText("Game Over(Press New Game)")
            return
